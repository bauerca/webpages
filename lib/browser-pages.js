...
var tick = require('process').nextTick;
...
extend(pages, {
  ...
  _cacheState: function(page) {
    if (page._cache) {
      history.replaceState(
        extend({state: page.state}, history.state),
        null,
        null
      );
    }
  ...
  window.addEventListener('popstate', function(event) {
    ...
    if (route) {
      if (route.state) {
        ...
        Page ? run(Page) : pages.fetch(route.name, run);
      }
      else {
        pages.request(
